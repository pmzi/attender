...
class LMSDriver {
    ...
    constructor({
      link, username, password, loginPage,
    }) {
      this.link = link;
    ...
    async bootstrap() {
      this.browser = LMSDriver.userBrowsers[this.username] ?? await puppeteer.launch({ headless: true, product: 'firefox' });
    ...
    async attend() {
      ...
      const classURL = LMSDriver.findClassUrl(onClick);
...
LMSDriver.findClassUrl = function findClassUrl(onClick) {
